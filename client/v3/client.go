...
type Client struct {
	Cluster     //集群接口，主要是集群服务器相关，可以通过rpc请求实现对集群节点的增删改查。
	KV          //KV接口，核心的key-value增删查改，是最常用的模块。
	Lease       //租约接口，用于管理自动续租的会话，基于该会话写入的key-value记录会被服务器保持到会话结束，会话正常结束或者续租失效，记录会被服务器自动清除。很多功能诸如服务发现、分布式锁等都可以基于该模块实现。
	Watcher     //Watcher接口，监听模块，用于监听记录的插入、修改和删除事件。
	Auth        //授权接口
	Maintenance //监控接口
	...
	resolver *resolver.EtcdManualResolver         //grpc resolver build用来设置endpoint
	mu       *sync.RWMutex
...
func (c *Client) credentialsForEndpoint(ep string) grpccredentials.TransportCredentials {
	//RequiresCredentials 返回给定端点是否需要凭据证书进行连接。
...
func newClient(cfg *Config) (*Client, error) {
	...
	client.KV = NewKV(client)                   //KV
	client.Lease = NewLease(client)             //租约
	client.Watcher = NewWatcher(client)         //Watcher,监听
	client.Auth = NewAuth(client)               //授权
	client.Maintenance = NewMaintenance(client) //监控
